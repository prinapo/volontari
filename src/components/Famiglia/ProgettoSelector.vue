  ...
  >
    <template #option="scope">
      <q-item v-bind="scope.itemProps">
        <q-item-section>
          <q-item-label>{{ scope.opt.AnnoBando }} — {{ [scope.opt.Cognome_Beneficiario, scope.opt.Nome_Beneficiario].filter(Boolean).join(' ') }} — {{ scope.opt.Eta }} anni</q-item-label>
        </q-item-section>
        <q-item-section side>
    ...
    <template #selected-item="scope">
      <div class="q-gutter-xs row items-center rounded-borders q-pa-xs">
        ...
        <q-item-label caption class="text-grey-7 q-ml-xs">
          €{{ scope.opt.Allocato }}
        </q-item-label>
      </div>
    </template>
