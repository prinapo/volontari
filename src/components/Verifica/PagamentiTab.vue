    ...
    <div class="row items-center q-gutter-sm q-mb-md">
      ...
      <template v-if="subTab === 'proposti'">
        <div v-for="a in store.associazioni" :key="a.Nome" class="text-caption q-mr-md">
          ...
          >: €{{ formatNumber(residuo(a.Nome)) }} disponibili
        </div>
      </template>
    </div>
    ...
    <template v-if="subTab === 'proposti'">
      ...
      >
        ...
        <template #body-cell-famiglia="props">
          <q-td :props="props">
            {{ props.row.Famiglia?.Nome_Famiglia || '—' }}
          </q-td>
        </template>
        <template #item="props">
            ...
            <q-card flat bordered>
              <q-card-section>
                ...
                <div v-else class="row q-gutter-xs q-mt-sm">
                  <q-btn flat dense icon="check_circle" color="positive" size="sm" @click="handlePagato(props.row)">
                    <q-tooltip>Pagato</q-tooltip>
                  </q-btn>
                  <q-btn flat dense icon="cancel" color="negative" size="sm" @click="handleFallito(props.row)">
                    <q-tooltip>Fallito</q-tooltip>
                  </q-btn>
                  <q-btn flat dense icon="block" color="grey" size="sm" @click="handleAnnullato(props.row)">
                    <q-tooltip>Annullato</q-tooltip>
                  </q-btn>
                </div>
              </q-card-section>
    ...
    <template v-if="subTab === 'falliti'">
      ...
      <div v-if="selectedFalliti.length > 0" class="row items-center q-gutter-sm q-mt-md">
        ...
        <q-btn
          color="primary"
          icon="playlist_add"
          label="Includi in nuovo batch"
          :disable="!batchAssociazioneFalliti"
          @click="creaBatchDaFalliti"
        />
      </div>
    </template>
...
<script setup>
  ...
  const selectedTotal = computed(() => selected.value.reduce((s, p) => s + (parseFloat(p.Importo) || 0), 0))
  ...
  async function exportCsv() {
    ...
    a.href = url
    a.download = `batch-${batchFilter.value || 'tutti'}.csv`
    a.click()
